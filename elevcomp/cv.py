...
Entry points:  run_cv_experiment.py (CLI)  and  run_ablations.py (variant plans).
...
HPARAM_OVERRIDES = [
    ...
    'model', 'epochs', 'batch_size', 'lr', 'weight_decay', 'grad_clip',
    'layer_size', 'base_channels', 'amp', 'amp_dtype',
    # loss
...
def load_base_config(overrides: dict = None) -> dict:
    """configs/cv_resnet34.toml + overrides → experiment config."""
    cfg_path = CONFIG_DIR / 'cv_resnet34.toml'
    if not cfg_path.exists():
        raise FileNotFoundError(
            f'{cfg_path} not found — the cross-validation experiment reads '
            f'configs/cv_resnet34.toml (configs/single_run.toml belongs to '
            f'scripts/train_single.py).')
...
def write_tables(summary: dict, out_dir: Path):
    ...
    lines = [
        '% Auto-generated by run_cv_experiment.py — test metrics per held-out environment',
